...
class SensorRemoteDataSourceImpl implements SensorRemoteDataSource {
  ...
  Future<List<SensorModel>> getSensors() async {
    try {
      ...
      if (response.statusCode == 200) {
        final List<dynamic> data = response.data['data']; // Adjust based on API structure
  ...
  Future<List<SensorMeasure>> getSensorHistory(String id) async {
    try {
      final response = await dio.get('/capteurs/$id/mesures?limit=50&sort=desc'); // Adjusted path slightly if needed, but Controller said /:id/mesures
      if (response.statusCode == 200) {
        final List<dynamic> data = response.data['data'];
        return data.map((json) => SensorMeasure(
